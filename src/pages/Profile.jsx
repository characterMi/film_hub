    ...
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <Box display="flex" alignItems="center">
          <Avatar sx={{ border: '1px solid', p: 1, width: { xs: '45px', sm: '60px' }, height: { xs: '45px', sm: '60px' } }} alt={user?.username} src={`https://www.themoviedb.org/t/p/w64_and_h64_face/${user?.avatar?.tmdb?.avatar_path}`} />
        ...
        <Button size="small" sx={{ height: 'max-content' }} color={theme.palette.mode === 'light' ? "primary" : 'error'} variant="outlined" onClick={() => setAlertBox(true)}>
          Logout &nbsp; <ExitToApp />
        </Button>
      </Box>
      <Box mb="4rem" mt="4rem">
